  ...
  Future<void> _loadCategories() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Nie udało się załadować kategorii: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder:
          (context) => AlertDialog(
            ...
            content: Text(
              'Czy na pewno chcesz usunąć kategorię ${_isExpense ? 'wydatku' : 'przychodu'} "${category.name}"?',
            ),
            actions: [
              TextButton(
      ...
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'Kategoria ${_isExpense ? 'wydatku' : 'przychodu'} została usunięta',
              ),
              backgroundColor: Colors.green,
            ),
          );
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Nie udało się usunąć kategorii: $e'),
              backgroundColor: Colors.red,
            ),
          );
        }
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                Text(
                  'Kategorie ${_isExpense ? 'wydatków' : 'przychodów'}',
                    ...
                    ? Center(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text(
                            'Brak kategorii ${_isExpense ? 'wydatków' : 'przychodów'}',
                            style: Theme.of(context).textTheme.titleLarge
                                ?.copyWith(color: Colors.grey[600]),
                          ),
                          const SizedBox(height: 8),
                          Text(
                            'Dodaj pierwszą kategorię ${_isExpense ? 'wydatku' : 'przychodu'}, aby zacząć',
                    ...
                    : RefreshIndicator(
                      ...
                      child: ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          final categoryWithLevel =
                              _hierarchicalCategories[index];

                          return CategoryListItem(
                            categoryWithLevel: categoryWithLevel,
                            onEdit:
                                () => _showCategoryForm(
                                  category: categoryWithLevel.category,
                                ),
                            onDelete:
                                () =>
                                    _deleteCategory(categoryWithLevel.category),
                          );
                        },
                      ),
                    ),
