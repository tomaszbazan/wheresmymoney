...
class CategoryHierarchy {
  static List<CategoryWithLevel> buildHierarchy(List<Category> categories) {
    ...
    final topLevelCategories = categories.where((category) => category.isTopLevel).toList()..sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase()));
  ...
  static void _addCategoryWithChildren(Category category, List<Category> allCategories, List<CategoryWithLevel> result, int level) {
    result.add(CategoryWithLevel(category, level));

    final children = allCategories.where((child) => child.parentId == category.id).toList()..sort((a, b) => a.name.toLowerCase().compareTo(b.name.toLowerCase()));
