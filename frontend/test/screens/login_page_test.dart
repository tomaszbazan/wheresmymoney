...
void main() {
  group('Login error message handling', () {
    test('should return user-friendly message for invalid credentials', () {
      ...
      expect(
        getErrorMessage(Exception('Invalid credentials')),
        'Nieprawidłowy email lub hasło',
      );
      expect(
        getErrorMessage(Exception('Too many requests')),
        'Zbyt wiele prób logowania. Spróbuj ponownie za chwilę',
      );
      expect(
        getErrorMessage(Exception('Network error')),
        'Błąd połączenia z serwerem. Sprawdź połączenie internetowe',
      );
      expect(
        getErrorMessage(Exception('Some other error')),
