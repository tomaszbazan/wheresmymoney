...
public class JpaAuditLogRepository implements AuditLogRepository {
    ...
    public Optional<AuditLog> findById(AuditLogId id, GroupId groupId) {
        return repository.findByIdAndGroupId(id.value(), groupId.value())
    ...
    public Page<AuditLog> findByQuery(AuditLogQuery query, Pageable pageable) {
        Specification<AuditLogEntity> spec = (root, criteriaQuery, criteriaBuilder) -> {
            ...
            predicates.add(criteriaBuilder.equal(root.get("groupId"), query.groupId().value()));
            ...
            if (query.entityId() != null) {
                predicates.add(criteriaBuilder.equal(root.get("entityId"), query.entityId().value()));
            }
            ...
            if (query.performedBy() != null) {
                predicates.add(criteriaBuilder.equal(root.get("performedBy"), query.performedBy().value()));
            }
