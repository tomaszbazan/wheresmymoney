...
public record AuditLogsView(
        List<AuditLogView> auditLogs, int page, int size, long totalElements, int totalPages) {
    public AuditLogsView {
        auditLogs = List.copyOf(auditLogs);
    }

    public static AuditLogsView from(Page<AuditLog> auditLogPage) {
        var auditLogs = auditLogPage.getContent().stream().map(AuditLogView::from).toList();
