...
public class TransactionControllerTest {
    ...
    void shouldReturnTransactionById() throws Exception {
        ...
        var transaction = createTransaction(
                transactionId, accountId, new BigDecimal("100.50"), "Test transaction", TransactionType.INCOME);
        ...
        mockMvc.perform(get("/api/transactions/" + transactionId)
                ...
                .andExpect(jsonPath("$.amount.value").value(100.50))
                ...
                .andExpect(jsonPath("$.bill.items[0].amount.value").value(100.50))
                .andExpect(jsonPath("$.bill.items[0].amount.currency").value("PLN"))
    ...
    void shouldReturnListOfAllTransactions() throws Exception {
        ...
        var transaction1 = createTransaction(
                randomUUID(), randomUUID(), new BigDecimal("100.00"), "Transaction 1", TransactionType.INCOME);
        ...
        mockMvc.perform(get("/api/transactions")
                ...
                .andExpect(
                        jsonPath("$.transactions[0].bill.items[0].description").value("Transaction 1"))
                .andExpect(
                        jsonPath("$.transactions[1].bill.items[0].description").value("Transaction 2"))
    ...
    void shouldUpdateTransaction() throws Exception {
        ...
        var updatedTransaction = createTransaction(
                transactionId, accountId, new BigDecimal("150.00"), "Updated transaction", TransactionType.INCOME);
        ...
        mockMvc.perform(put("/api/transactions/" + transactionId)
                ...
                .andExpect(content().contentType(APPLICATION_JSON_UTF8_VALUE))
                .andExpect(jsonPath("$.id").value(transactionId.toString()))
                .andExpect(jsonPath("$.amount.value").value(150.00))
                .andExpect(jsonPath("$.amount.currency").value("PLN"))
                .andExpect(jsonPath("$.bill.items", hasSize(1)))
                .andExpect(jsonPath("$.bill.items[0].description").value("Updated transaction"))
                .andExpect(jsonPath("$.bill.items[0].amount.value").value(150.00))
                .andExpect(jsonPath("$.bill.items[0].amount.currency").value("PLN"));
    }
    ...
    private Transaction createTransaction(
            UUID transactionId, UUID accountId, BigDecimal amount, String description, TransactionType type) {
        ...
        var hash = TransactionHashCalculator.calculateHash(accountIdObj, money, description, transactionDate, type);
